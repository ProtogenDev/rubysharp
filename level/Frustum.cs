...
namespace mc_c_.level
{
    public class Frustum
    {
        ...
        public Frustum()
        {
            for (int i = 0; i < 6; ++i) m_Frustum[i] = new float[4];
        }
        ...
        private unsafe void CalculateFrustum()
        {
            var gl = GlState.Gl;

            // TODO-VERIFY: exact Silk.NET.OpenGL.Legacy method name/overload for
            // glGetFloatv(GL_PROJECTION_MATRIX/GL_MODELVIEW_MATRIX, ...) -
            // confirm via IDE intellisense (likely gl.GetFloat(GLEnum, float*)
            // or gl.GetFloat(GetPName, Span<float>)).
            fixed (float* projPtr = proj)
            fixed (float* modlPtr = modl)
            {
                gl.GetFloat(GLEnum.ProjectionMatrix, projPtr);
                gl.GetFloat(GLEnum.ModelviewMatrix, modlPtr);
            }
